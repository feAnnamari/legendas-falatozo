        ...
        <div class="mt-2 lg:sticky top-16 w-full lg:max-w-[260px]">
          <section class="my-8 flex gap-4 items-start justify-center w-full">
            ...
            <div class="flex-1 text-balance leading-tight w-full text-ellipsis overflow-hidden">
              ...
              <span v-if="viewer?.email" class="text-stone-400 font-light" :title="viewer?.email">{{ viewer?.email }}</span>
            </div>
            <button class="flex text-stone-700 items-center flex-col p-2 px-4 rounded-lg hover:bg-white hover:text-red-700 lg:hidden" @click="logoutUser">
          ...
          <hr class="my-8" />
          <nav class="flex text-stone-700 lg:grid flex-wrap w-full gap-1.5 my-8 min-w-[240px] lg:w-auto items-start">
          ...
          <template class="hidden lg:block">
            <hr class="my-8" />
            <button class="flex text-stone-700 items-center gap-4 p-2 px-4 w-full rounded-lg hover:bg-white hover:text-red-700" @click="logoutUser">
              <LoadingIcon v-if="isPending" size="22" />
              <Icon v-else name="ion:log-out-outline" size="22" />
