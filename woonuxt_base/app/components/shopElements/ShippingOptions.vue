...
<template>
  <div class="grid gap-4 shipping-options">
    <div
      ...
      <div>
        <div class="text-sm leading-tight text-stone-500" v-html="option.label"></div>
        <div class="font-semibold text-stone-600">{{ currencySymbol }}{{ option.cost }}</div>
      </div>
...
.shipping-options {
  ...
  .option {
    @apply bg-white border rounded-lg text-stone-600 cursor-pointer flex flex-1 text-sm py-3 px-4 gap-2 items-center hover:border-purple-300;
