...
<template>
  <form class="relative items-center flex-1 -space-x-px rounded-md shadow-sm" @submit.prevent="setSearchQuery(searchQuery)">
    ...
    <input
      ...
      class="z-0 inline-flex items-center w-full p-2 pl-10 text-sm text-stone-500 border border-stone-300 rounded-md shadow-inner outline-none bg-stone-50 shadow-stone-200" />
    <span
      v-if="searchQuery"
