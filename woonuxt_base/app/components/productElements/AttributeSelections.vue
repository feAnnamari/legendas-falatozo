  ...
  <div class="flex flex-col gap-1 justify-between" v-if="attributes">
    <div v-for="(attr, i) in attributes" :key="i" class="flex flex-wrap py-2 relative justify-between">
      ...
      <div v-if="attr.scope == 'LOCAL'" class="grid gap-2">
        <div class="text-sm">
          {{ attr.label }}
          <span v-if="activeVariations.length && activeVariations[i]" class="text-stone-400">: {{ getSelectedName(attr, activeVariations[i]) }}</span>
        </div>
      ...
      <div v-else-if="attr.name == 'pa_color' || attr.name == 'color'" class="grid gap-2">
        <div class="text-sm">
          ...
          <span v-if="activeVariations.length" class="text-stone-400">{{ getSelectedName(attr, activeVariations[i]) }}</span>
        </div>
        <div class="flex gap-2">
      ...
      <div v-else-if="attr.terms.nodes && attr.terms.nodes?.length > 8" class="grid gap-2">
        <div class="text-sm">
          {{ attr.label }} <span v-if="activeVariations.length" class="text-stone-400">{{ getSelectedName(attr, activeVariations[i]) }}</span>
        </div>
...
<style lang="postcss">
.radio-button {
  @apply border-transparent border-white rounded-lg cursor-pointer outline bg-stone-50 border-2 text-sm text-center outline-2 outline-stone-100 py-1.5 px-3 transition-all text-stone-800 inline-block hover:outline-stone-500;
}

.color-button {
  @apply border-transparent border-white cursor-pointer outline bg-stone-50 border-2 rounded-2xl text-sm text-center outline-2 outline-stone-100 transition-all text-stone-800 inline-block hover:outline-stone-500;
  width: 2rem;
  height: 2rem;
}
...
input[type='radio']:checked ~ span {
  @apply outline outline-2 outline-stone-500;
}
</style>
