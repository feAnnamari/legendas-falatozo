...
<template>
  <div>
    <h4 v-if="reviews.edges.length" class="font-semibold text-2xl text-stone-900">{{ $t('messages.shop.customerReviews') }}</h4>
    <h4 v-else class="font-semibold text-2xl text-stone-900">{{ $t('messages.shop.noReviews') }}</h4>
    ...
    <div class="my-4 bars">
      <div v-for="rating in numberAndPercentageOfEachRating" :key="rating" class="flex gap-4 items-center">
        ...
        <div class="flex-1 relative">
          <div class="rounded-full bg-stone-200 h-2.5 w-full"></div>
          <div class="rounded-full bg-yellow-400 h-2.5 top-0 left-0 absolute" :style="{ width: rating.percentage + '%' }"></div>
        </div>
      </div>
    </div>
    <div class="mt-10 text-xl mb-2 text-stone-900">Share your thoughts</div>
...
.disable-star {
  @apply bg-white shadow-sm text-stone-300 border border-stone-300;
